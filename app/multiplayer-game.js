...
export default function MultiplayerGame() {
  ...
  useEffect(() => {
    console.log(
      `WebSocket connection status changed in multiplayer-game: ${
  ...
  useEffect(() => {
    if (wsError) {
      ...
      if (
        ...
        wsError.includes("Connection error")
      ) {
    ...
    if (params.returningFromGame === "true" && params.gameId) {
      ...
      // Otherwise try to connect only if not loading and not in a connection attempt
        ...
        if (now - lastReconnectAttempt.current < 3000) {
          console.log(
            "Skipping reconnection attempt - too soon since last attempt"
          );
          return;
        }
        ...
        connect(serverUrl || undefined).then((connectedSocket) => {
          if (connectedSocket) {
            ...
            setError(
              "Failed to reconnect to the game server. Please try again."
            );
            setConnectionStep("initial");
          }
          setLoading(false);
    ...
    const gameStartedCleanup = on(EVENTS.GAME_STARTED, (data) => {
      ...
      router.push({
        ...
        params: {
          ...
          gameId: data.game.id,
          players: JSON.stringify(data.game.players),
          isMultiplayer: "true",
  ...
  const handleCreateGame = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `Creating game with ${
          playerTracks.length
        } host tracks and profile info: ${userProfile ? "Yes" : "No"}`
      );
  ...
  const handleJoinGame = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `Attempting to join game ${joinCode} as ${username} with ${playerTracks.length} tracks...`
      );
      ...
      if (joinSuccess) {
        ...
        setTimeout(() => {
          if (connectionStep !== "lobby" && loading) {
            setError(
              "Joining timed out. The game may not exist or the server may be unreachable."
            );
            setLoading(false);
          }
        }, 10000);
        ...
        setError(
          "Failed to join game. Please check your connection and try again."
        );
        setLoading(false);
      }
    } catch (err) {
  ...
  const renderLobbyScreen = () => {
    ...
    const joinUrl = `synth://join?gameId=${gameId}&serverUrl=${encodeURIComponent(
      serverUrl || "http://10.32.170.64:3000"
